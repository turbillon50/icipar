    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        <Link href={variant === 'admin' ? '/admin' : '/app'} className="flex items-center gap-3">
          ...
          {variant === 'admin' && (
            <span className="text-xs text-red-400 border border-red-400/30 px-2 py-0.5 rounded-full">Admin</span>
          )}
        </Link>
          ...
          {links.map(l => (
            <Link key={l.href} href={l.href}
              className={`text-sm transition-colors ${
                pathname === l.href ? 'text-amber-400' : 'text-stone-400 hover:text-white'
              }`}>{l.label}</Link>
          ))}
        </div>
        ...
        >
          {links.map(l => (
            <Link key={l.href} href={l.href} onClick={() => setOpen(false)}
              className="block px-6 py-3 text-stone-300 hover:text-amber-400 hover:bg-amber-900/10 transition-colors text-sm">
              {l.label}
            </Link>
          ))}
        </motion.div>
